...
router.get('/stats', (req, res) => {
  ...
  const whereClause = query ? 'WHERE first_name LIKE ? OR last_name LIKE ? OR email LIKE ?' : '';
  ...
  db.get(countSql, whereParams, (countErr, countRow) => {
    if (countErr) {
      return sendError(res, 500, 'Erreur lors du comptage des joueurs.', countErr.message);
    }

    db.all(playersSql, whereParams, (playersErr, playersRows) => {
      if (playersErr) {
        return sendError(res, 500, 'Erreur lors de la récupération des joueurs.', playersErr.message);
      }
        ...
        ranked AS (
          SELECT
            ...
            ROW_NUMBER() OVER (
              ...
              ORDER BY datetime(played_at) DESC, match_id DESC
            ) AS rn
          FROM player_results
        )
        SELECT player_id, result
        FROM ranked
        WHERE rn <= 5 AND player_id IN (${placeholders})
        ORDER BY player_id ASC, rn ASC
      `;

      db.all(statsSql, playerIds, (statsErr, statsRows) => {
        if (statsErr) {
          return sendError(res, 500, 'Erreur lors du calcul des statistiques.', statsErr.message);
        }

        db.all(recentFormSql, playerIds, (formErr, formRows) => {
          if (formErr) {
            return sendError(res, 500, 'Erreur lors du calcul de la forme récente.', formErr.message);
          }
          ...
          const safeLimit = Number.isInteger(limit) && limit > 0 ? limit : total;
